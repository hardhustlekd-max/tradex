...
export const PairSelectorModal: React.FC<PairSelectorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-2xl liquid-card rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] font-mono text-zinc-100 animate-in fade-in zoom-in-95 duration-100" onClick={(e) => e.stopPropagation()}>
        {/* Modal Header */}
        <div className="p-4 border-b border-white/10 flex items-center justify-between bg-[#181a20]">
          <h2 className="text-base font-bold flex items-center gap-2">
            <span className="text-[#00c076]">Select Trading Market</span>
        ...
        {/* Search & Categories */}
        <div className="p-4 border-b border-white/10 space-y-3 bg-[#0d1117]/90">
          {/* Search Input */}
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3 top-2.5 text-[#00c076]" />
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search coin name (e.g. BTC, SOL, ETH)..."
              className="w-full pl-9 pr-4 py-2 bg-[#161b22] border border-white/10 focus:border-[#00c076] rounded-lg text-xs font-mono text-zinc-100 focus:outline-none tabular-nums"
            />
          </div>
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-3 py-1 text-xs font-semibold cursor-pointer transition-all ${
                  selectedCategory === cat ? 'app-subtab-active' : 'app-subtab-inactive'
                }`}
              >
                {cat}
              </button>
                ...
                return (
                  <tr
                    ...
                    className={`hover:bg-white/5 cursor-pointer transition-colors ${
                      isActive ? 'bg-white/10 font-bold border-l-2 border-[#00c076] text-white' : ''
                    }`}
                  >
                    <td className="p-2.5">
                    ...
                    <td className="p-2.5 text-right font-mono text-zinc-200 tabular-nums">
                      {formatCurrency(pair.price, pair.precision)}
                    </td>

                    <td className={`p-2.5 text-right font-mono font-semibold tabular-nums ${isPositive ? 'text-[#00c076]' : 'text-[#f6465d]'}`}>
                      {isPositive ? '+' : ''}{pair.change24h.toFixed(2)}%
                    </td>

                    <td className="p-2.5 text-right font-mono text-zinc-400 tabular-nums">
